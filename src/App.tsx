        ...
        <div className="max-w-md w-full mx-auto animate-in fade-in slide-in-from-bottom lg:slide-in-from-right duration-700 py-12">
           ...
           <div className="space-y-8">
              ...
              <div className="pt-4 text-center border-t border-slate-50">
                 ...
                 <p className="mt-6 text-[9px] font-black text-slate-200 uppercase tracking-widest">Admin & Campus Only</p>
              </div>
           </div>
        </div>
      </div>
        ...
        <div className="max-w-md w-full mx-auto animate-in slide-in-from-bottom duration-700">
           ...
           <div className="space-y-6">
                ...
                <div key={f.key} className="space-y-1.5 group">
                   ...
                   <div className="relative">
                      <input type={f.type} value={(formData as any)[f.key]} onChange={e => setFormData({ ...formData, [f.key]: e.target.value })} 
                        ...
                        className="w-full bg-slate-50 border-2 border-slate-50 px-7 py-4 rounded-[1.5rem] text-slate-800 text-sm font-bold focus:outline-none focus:border-rose-400 focus:bg-white transition-all shadow-inner placeholder:text-slate-200" placeholder={f.label} />
              ...
              <button onClick={() => {
                ...
                onSignup(formData);
              }} className="w-full py-5 bg-emerald-500 text-white rounded-[2rem] font-black uppercase tracking-[0.3em] shadow-xl hover:bg-emerald-600 active:scale-[0.98] transition-all text-sm mt-4">계정 생성 요청하기</button>
...
export default function App() {
  ...
  const [registeredCampuses, setRegisteredCampuses] = useState<Campus[]>(() => {
    try {
      const s = localStorage.getItem('eie_campuses');
      if (s) return JSON.parse(s);
    } catch(e) {}
    return [{ region: '서울', name: '신촌 캠퍼스' }, { region: '서울', name: '강남 캠퍼스' }];
  });

  useEffect(() => { localStorage.setItem('eie_users_v2', JSON.stringify(allUsers)); }, [allUsers]);
