...
import AdminReadingLevelSetup from './components/AdminReadingLevelSetup';
import AdminReadingLevelResults from './components/AdminReadingLevelResults';
import AdminGrammarLevelSetup from './components/AdminGrammarLevelSetup';
import AdminGrammarLevelResults from './components/AdminGrammarLevelResults';
...
const games = [
   ...
   { id: 'reading-certification', title: '리딩레벨', subtitle: 'Reading Level', icon: '📚', gradient: 'from-emerald-600 to-emerald-400', desc: '독해력을 인증받고 보상을 획득하는 성장 미션', img: '/assets/games/reading-level.png', tag: 'Level' },
   { id: 'grammar-certification', title: '그래머레벨', subtitle: 'Grammar Level', icon: '✍️', gradient: 'from-amber-600 to-amber-400', desc: '문법력을 인증받고 보상을 획득하는 성장 미션', img: '/assets/games/grammar-level.png', tag: 'Level' },
];
...
const AdminDashboard = ({ campusUsers, updateLevel, onBulkLevelUpdate, defaultCampusLevel, onUpdateDefaultLevel, onDeleteCampus, onBulkRegister, onSingleRegister, onResetAll, onLogout, registeredCampuses, user, gameConfigs, onUpdateGameConfig, onEditCampusInfo }: any) => {
   ...
   const [activeTab, setActiveTab] = useState<'home' | 'approvals' | 'campuses' | 'games' | 'stats' | 'word-levels' | 'word-results' | 'reading-levels' | 'reading-results' | 'grammar-levels' | 'grammar-results'>('home');
      ...
      <div className="h-screen bg-slate-50 text-slate-800 flex overflow-hidden font-sans selection:bg-indigo-100">
         <aside className="w-60 bg-slate-900 flex flex-col z-30 shadow-2xl shrink-0 no-print">
            ...
            <nav className="flex-1 p-4 space-y-1 mt-6">
               {[
                  ...
                  { id: 'word-results', icon: ClipboardList, label: 'Word Results' },
                  { id: 'reading-levels', icon: BookOpen, label: 'Reading Levels' },
                  { id: 'reading-results', icon: ClipboardList, label: 'Reading Results' },
                  { id: 'grammar-levels', icon: BookOpen, label: 'Grammar Levels' },
                  { id: 'grammar-results', icon: ClipboardList, label: 'Grammar Results' },
         ...
         <main className="flex-1 overflow-hidden p-6 lg:p-8 relative h-screen bg-slate-100 flex flex-col">
            <div className="max-w-[1400px] w-full h-full mx-auto flex flex-col min-h-0">
               ...
               ) : activeTab === 'reading-levels' ? (
                  <AdminReadingLevelSetup />
               ) : activeTab === 'reading-results' ? (
                  <AdminReadingLevelResults />
               ) : activeTab === 'grammar-levels' ? (
                  <AdminGrammarLevelSetup />
               ) : activeTab === 'grammar-results' ? (
                  <AdminGrammarLevelResults />
               ) : null}
            </div>
                  ...
                  <div className="h-full flex flex-col">
                     <div className="flex-1 bg-white rounded-[3rem] p-4 overflow-hidden shadow-2xl">
                        {selectedGame === 'word-chain' ? (<WordChain />) :
                           ...
                           selectedGame === 'reading-certification' ? (<ReadingLevel onBack={() => setSelectedGame(null)} maxLevel={7} user={user} />) :
                           selectedGame === 'grammar-certification' ? (<GrammarLevel onBack={() => setSelectedGame(null)} maxLevel={10} user={user} />) :
