...
export default function BingoGame() {
  ...
  const isReady = words.length >= requiredCount;
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        setWords(imported.sort(() => Math.random() - 0.5));
            ...
            <div className="grid grid-cols-2 gap-3">
               <div className="bg-white border border-slate-200 rounded-[1.5rem] p-5 shadow-sm">
                  <div className="flex justify-between items-center mb-4">
                     ...
                     <span className="text-2xl font-black text-emerald-600 italic tracking-tighter">{gridSize}×{gridSize}</span>
                  </div>
                  <div className="flex gap-2">
                    {[3, 4, 5, 6].map(size => (
                      <button key={size} onClick={() => { setGridSize(size); setTargetBingo(Math.min(targetBingo, size)); }}
                        className={`flex-1 py-3 rounded-xl text-xl font-[1000] border-2 transition-all ${gridSize === size ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-slate-50 border-slate-100 text-slate-300'}`}>
                        {size}
                      </button>
                    ))}
                  </div>
               ...
               <div className="bg-white border border-slate-200 rounded-[1.5rem] p-5 shadow-sm">
                  <div className="flex justify-between items-center mb-4">
                     <label className="text-sm font-black text-slate-400 uppercase tracking-widest">목표 빙고</label>
                     <span className="text-2xl font-black text-emerald-600 italic tracking-tighter">{targetBingo} L</span>
                  </div>
                  <div className="flex gap-1 overflow-x-auto custom-scrollbar-light pb-1">
                    {[1, 2, 3, 4, 5, 6, 7, 8].map(n => (
                      <button key={n} onClick={() => setTargetBingo(n)}
                        className={`min-w-[40px] py-2.5 rounded-lg border-2 text-base font-black transition-all ${targetBingo === n ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-slate-50 border-slate-100 text-slate-200'}`}>
                        {n}
                      </button>
                    ))}
                  </div>
  ...
  return (
    <div className="max-w-4xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-500 py-3 h-full flex flex-col text-slate-900 font-sans overflow-hidden">
      <div className="flex items-center justify-between mb-4 px-6 bg-white border border-slate-200 rounded-2xl py-4 shadow-sm">
        ...
        <h2 className="text-2xl font-[1000] text-slate-900 italic uppercase tracking-tighter border-l-4 border-emerald-500 pl-4 leading-none">BINGO<span className="text-emerald-500 text-sm ml-2">{gridSize}×{gridSize}</span></h2>
