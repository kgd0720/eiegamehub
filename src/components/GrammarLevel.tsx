...
export default function GrammarLevel({ onBack, maxLevel = 10, user }: GrammarLevelProps) {
   ...
   const [timeLeft, setTimeLeft] = useState(getGrammarTimeLimit());
      ...
      import('../../lib/api').then(api => {
         Promise.all([api.getGrammarLevels(), api.getGlobalSettings()]).then(([parsed, settings]) => {
            if (parsed && Array.isArray(parsed) && parsed.length > 0) {
               ...
               try {
                  const stored = localStorage.getItem('eie_grammar_level_dict');
                  if (stored) {
      ...
      if (gameState === 'result' && !hasSavedResult.current) {
         ...
         import('../../lib/api').then(api => {
            api.saveGrammarLevelResult({
               ...
               score: levelScore,
               total_questions: levelQuestions.length || 10
            }).then(success => {
               if (success) console.log('Successfully saved grammar level test result.');
            });
   ...
   const startLevel = (lvl: number) => {
      ...
      const shuffled = shuffleArray(uniqueQList).slice(0, 10);
      ...
      setTimeLeft(getGrammarTimeLimit());
         ...
         <div className="w-full h-full min-h-0 flex items-center justify-center p-3 sm:p-6 font-sans animate-in fade-in text-slate-900 overflow-hidden">
            <div className="max-w-[500px] w-full bg-white border border-slate-200 rounded-[2.25rem] shadow-[0_8px_30px_rgba(0,0,0,0.05)] flex flex-col p-5 sm:py-11 sm:px-8 relative gap-4.5 sm:gap-6 justify-center animate-in zoom-in-95 duration-200">
               ...
               <div className="flex flex-col items-center mb-0.5 sm:mb-1.5">
                  <div className="w-10 h-10 sm:w-11 sm:h-11 bg-amber-50 rounded-xl sm:rounded-2xl flex items-center justify-center text-xl sm:text-2xl mb-2 sm:mb-2.5 shadow-sm border border-amber-100">✍️</div>
                  <h1 className="text-lg sm:text-2xl font-[1000] text-slate-900 mb-0.5 sm:mb-1 tracking-tight">문법 레벨 테스트</h1>
                  <p className="text-[11px] sm:text-sm font-bold text-slate-500">전국의 <span className="text-amber-600 font-[1000]">EIE</span> 학생들과 함께하는 어법 실력 검정</p>
               </div>
               ...
               <div className="w-full flex flex-col gap-0.5 sm:gap-1">
                  ...
                  <div className="bg-white border border-slate-200/80 rounded-xl p-3.5 sm:py-4.5 sm:px-4 mb-2 shadow-sm">
                     ...
                     <input 
                        ...
                        className="w-full bg-white border border-slate-200 focus:border-[#D97706] focus:ring-4 focus:ring-[#D97706]/15 outline-none transition-all px-3 py-2 sm:px-4 sm:py-3 rounded-lg text-xs sm:text-base font-black text-amber-950 placeholder:text-slate-400" 
                           ...
                           return (
                              <button 
                                 ...
                                 className={`py-2 sm:py-3.25 rounded-lg sm:rounded-xl font-[1000] text-xs sm:text-sm transition-all duration-200 border ${
                                    isSelected 
                                       ? 'bg-[#D97706] text-white border-[#D97706] shadow-sm scale-[1.02]' 
                                       : 'bg-white text-slate-600 border-slate-200 hover:border-[#D97706]/40 hover:bg-slate-50'
                                 }`}
                              >
                                 {g}
                              </button>
                  ...
                  <button 
                     ...
                     className={`w-full py-3 sm:py-4 rounded-xl sm:rounded-2xl font-[1000] text-sm sm:text-lg uppercase tracking-widest text-white transition-all duration-300 mt-2 flex items-center justify-center gap-2 ${
                        isReady && playerInfo.name.trim() && playerInfo.grade
                           ? 'bg-[#D97706] hover:scale-[1.02] shadow-[0_3px_12px_rgba(217,119,6,0.35)] cursor-pointer' 
   ...
   if (gameState === 'interstitial') {
      ...
      return (
         <div className="max-w-4xl mx-auto w-full h-full flex items-center justify-center p-4 py-8 animate-in slide-in-from-bottom duration-500 font-sans">
            <div className={`w-full max-w-2xl border-8 rounded-[3rem] p-8 lg:p-12 text-center shadow-2xl ${passed ? 'bg-white border-amber-500' : 'bg-white border-rose-500'}`}>
               <div className="text-[64px] mb-4 drop-shadow-lg">{passed ? '🎉' : '💔'}</div>
               <h2 className={`text-3xl lg:text-4xl font-[1000] tracking-tighter uppercase italic leading-none mb-3 ${passed ? 'text-amber-500' : 'text-rose-500'}`}>
                  {passed ? 'LEVEL CLEAR!' : 'LEVEL FAILED'}
               </h2>
               <p className="text-xl font-black text-slate-700 mb-6 uppercase tracking-widest">
                  G{currentLevel} <span className="text-slate-300 mx-2">|</span> <span className="whitespace-nowrap">정답: {levelScore} / {levelQuestions.length}</span>
               </p>

               <div className="bg-slate-50 border border-slate-100 p-5 rounded-3xl mb-8 text-slate-500 font-bold text-sm">
                  {passed ? '축하합니다! 다음 단계에 도전할 자격을 획득했습니다.' : (timeLeft <= 0 ? '시간이 초과되어 도전이 종료되었습니다. (8문제 이상 정답 필요)' : '아쉽습니다. 다음 단계에 도전하려면 8개 이상 맞혀야 합니다.')}
               </div>

               {passed ? (
                  <button 
                     onClick={handleNextLevelFlow} 
                     className="w-full py-5 rounded-full font-[1000] text-xl uppercase tracking-widest text-white shadow-xl bg-amber-500 hover:scale-105 transition-all"
                  >
                     {currentLevel >= Math.min(10, maxLevel) ? '최대 레벨 도달 (결과 확인)' : '다음 레벨 도전'}
                  </button>
               ) : (
                  <button 
                     onClick={handleNextLevelFlow} 
   ...
   if (gameState === 'result') {
      return (
         <div className="fixed inset-0 z-[100] bg-[#14120c]/95 backdrop-blur-3xl flex items-center justify-center p-4 sm:p-6 overflow-y-auto animate-in fade-in duration-700 font-sans gen-result-screen-overlay">
            <div className="bg-white border-[12px] border-amber-500/20 rounded-[3rem] p-10 lg:p-14 max-w-xl w-full text-center shadow-2xl relative animate-in zoom-in-95 gen-result-card">
               <div className="absolute -top-16 left-1/2 -translate-x-1/2 w-32 h-32 bg-amber-600 rounded-full border-8 border-white flex items-center justify-center text-5xl shadow-xl gen-result-trophy">🏆</div>
               ...
               <div className="mt-14 mb-8 gen-result-title-section">
                  ...
                  <p className="text-amber-500 font-black uppercase tracking-[0.4em] text-sm">Grammar Level Test Report</p>
               </div>

               <div className="bg-amber-50 rounded-[2rem] p-8 mb-10 text-left border border-amber-100 shadow-inner gen-result-info-box">
                  <div className="flex items-end justify-between border-b-2 border-amber-100 pb-4 mb-4">
                     <span className="text-slate-400 font-black text-xs uppercase tracking-widest">학생 정보</span>
                     <div className="text-right leading-none">
                        <span className="text-2xl font-[1000] text-amber-900">{playerInfo.name}</span>
                        <span className="text-sm font-black text-amber-400 ml-2">({playerInfo.grade})</span>
                     </div>
                  </div>

                  <div className="flex items-center justify-between py-2">
                     <span className="text-slate-500 font-black text-sm">최종 도달 레벨</span>
                     <span className="text-3xl font-[1000] italic text-amber-600">G{currentLevel}</span>
                  </div>

                  <div className="flex items-center justify-between py-2 mt-2">
                     <span className="text-slate-500 font-black text-sm">학년 수준</span>
                     <span className="text-lg font-[900] text-amber-500">
                        {currentLevel === 1 ? 'G1 수준' :
                           currentLevel === 2 ? 'G2 수준' :
                              currentLevel === 3 ? 'G3 수준' :
                                 currentLevel === 4 ? 'G4 수준' :
                                    currentLevel === 5 ? 'G5 수준' :
                                       currentLevel === 6 ? 'G6 수준' :
                                          currentLevel === 7 ? 'G7 수준' :
                                             currentLevel === 8 ? 'G8 수준' :
                                                currentLevel === 9 ? 'G9 수준' :
                                                   currentLevel >= 10 ? 'G10 수준' : ''}
                     </span>
                  </div>

                  <div className="flex items-center justify-between py-2 mt-2">
                     <span className="text-slate-500 font-black text-sm">캠퍼스명</span>
                     <span className="text-lg font-black text-amber-900">{user?.name || '지정되지 않음'}</span>
                  </div>
               ...
               <div className="flex gap-4">
                  ...
                  <button onClick={onBack} className="flex-1 py-5 rounded-[2rem] bg-amber-600 text-white font-black uppercase tracking-widest text-lg hover:bg-amber-700 shadow-xl transition-all gen-result-btn">대시보드</button>
               </div>
            </div>
         </div>
      );
         ...
         <div className="w-full sticky top-0 z-50 bg-white/95 backdrop-blur-md border border-slate-200 rounded-3xl shadow-sm shrink-0 mb-1 flex flex-col overflow-hidden relative">
            ...
            <div className="grid grid-cols-5 border-b border-slate-200/80 w-full">
               ...
               <div className="flex flex-col items-center justify-center py-2 px-1 sm:py-3 sm:px-2 gap-0.5 border-l border-slate-200/80">
                  ...
                  <span className="text-[13px] sm:text-base md:text-lg font-[900] text-[#D97706] whitespace-nowrap leading-none mt-1">G{currentLevel}</span>
               </div>
               ...
               <div className="flex flex-col items-center justify-center py-2 px-1 sm:py-3 sm:px-2 gap-0.5 border-l border-slate-200/80">
                  ...
                  <span className={`text-[13px] sm:text-base md:text-lg font-[900] whitespace-nowrap leading-none mt-1 transition-all duration-300 ${
                     timeLeft <= 10 ? 'text-red-500 animate-pulse-red' : 'text-[#D97706]'
                  }`}>
               ...
               <div className="flex flex-col items-center justify-center py-2 px-1 sm:py-3 sm:px-2 gap-0.5 border-l border-slate-200/80">
                  ...
                  <span className="text-[13px] sm:text-base md:text-lg font-[900] text-orange-500 whitespace-nowrap leading-none mt-1">{levelScore} / 10</span>
               </div>
            </div>
            ...
            <div className="w-full h-1 bg-slate-50 relative overflow-hidden">
               <div 
                  className="h-full bg-gradient-to-r from-[#D97706] to-amber-400 rounded-full transition-all duration-300"
         ...
         <div className="w-full flex-1 min-h-0 bg-slate-50 border border-slate-200 rounded-[2.5rem] flex flex-col md:flex-row shadow-inner overflow-hidden max-h-[500px]">
            ...
            <div className="flex-[1.2] flex flex-col p-6 sm:p-10 justify-center items-center bg-white border-b md:border-b-0 md:border-r border-slate-200/80 relative">
               <div className="absolute top-4 left-6 flex items-center gap-2">
                  ...
                  <span className="text-[10px] sm:text-xs font-black uppercase text-amber-500 tracking-widest italic">Mission {String(currentQIdx + 1).padStart(2, '0')}</span>
               </div>
               
               <div className="w-full text-center px-4">
                  <h3 className="text-2xl sm:text-4xl md:text-5xl font-[1000] text-[#D97706] leading-tight tracking-tight max-w-[420px] mx-auto break-all py-4">
                     {q?.q}
                  </h3>
               </div>
               ...
               <button 
                  ...
                  className="absolute bottom-4 right-6 text-[10px] sm:text-xs font-black text-slate-400 hover:text-amber-500 transition-colors uppercase tracking-widest"
               >
                  다음 문제 ➔
               </button>
            </div>
                        ...
                        >
                           <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center font-black text-xs sm:text-sm shrink-0 border ${
                              isAnsweringSelected
                                 ...
                                 : isSelected ? 'bg-[#fffbeb] border-[#D97706] text-[#D97706]' : 'bg-slate-50 border-slate-200 text-slate-400'
                           }`}>
                              {idx + 1}
                           </div>
