        ...
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-2 items-stretch flex-1 overflow-y-auto custom-scrollbar-light pb-10 lg:pb-0 min-h-0">
          <div className="col-span-1 lg:col-span-7 flex flex-col gap-2 overflow-visible lg:overflow-y-auto min-h-0">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 shrink-0">
                <div className={`bg-white border rounded-[2.5rem] px-6 py-4 shadow-sm flex flex-col shrink-0 transition-all duration-300 ${matchMode ? 'border-indigo-500 ring-4 ring-indigo-500/5' : 'border-slate-200'}`}>
                  <div className="flex items-center justify-between mb-6">
                    <label className="text-[0.6875rem] font-[1000] text-indigo-900 uppercase tracking-widest bg-indigo-50 px-3 py-1 rounded-full">STEP 01. 대전 모드 선택</label>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => setMatchMode('single')}
                      ...
                      <span className={`text-[0.625rem] font-bold ${matchMode === 'single' ? 'text-indigo-100' : 'text-slate-300'}`}>플레이어들 VS 진행자</span>
                    </button>
                    <button onClick={() => setMatchMode('team')}
                      ...
                      <span className={`text-[0.625rem] font-bold ${matchMode === 'team' ? 'text-indigo-100' : 'text-slate-300'}`}>팀 간 대결</span>
                    </button>
                  </div>
                </div>

                <div className={`bg-white border rounded-[2.5rem] px-6 py-4 shadow-sm flex flex-col shrink-0 transition-all duration-300 ${targetNum ? 'border-indigo-500 ring-4 ring-indigo-500/5' : 'border-slate-200'}`}>
                  <div className="flex items-center justify-between mb-4">
                    <label className="text-[0.6875rem] font-[1000] text-indigo-900 uppercase tracking-widest bg-indigo-50 px-3 py-1 rounded-full">STEP 02. 목표 숫자 설정</label>
                    <span className="text-[0.625rem] font-black text-slate-400">RANGE: 1-100</span>
                  </div>
                  ...
                  <div className="flex-1 flex flex-col justify-center gap-6">
                    <div className="flex items-center justify-between bg-slate-50 border border-slate-100 p-4 rounded-2xl shadow-inner">
                       <div className="text-left">
                          <p className="text-[1.5rem] font-[1000] text-indigo-600 leading-none italic">{targetNum}</p>
                          <p className="text-[0.5625rem] font-bold text-slate-400 uppercase tracking-widest mt-1">Current Target</p>
                       </div>
                      ...
                      <div className="flex justify-between mt-2 px-1">
                        {[1, 25, 50, 75, 100].map(v => (
                          <div key={v} className="flex flex-col items-center gap-1">
                            ...
                            <span className={`text-[0.5rem] font-black ${targetNum === v ? 'text-indigo-600' : 'text-slate-300'}`}>{v}</span>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            <div className={`bg-white border rounded-[2.5rem] px-6 py-4 shadow-sm min-h-0 flex-1 flex flex-col transition-all duration-300 ${step3Done ? 'border-indigo-500 ring-4 ring-indigo-500/5' : 'border-slate-200'}`}>
               ...
               <div className="flex-1 max-h-[60vh] overflow-y-auto bg-slate-50/50 rounded-[1.5rem] border border-slate-100 p-4 flex flex-wrap content-start gap-2 custom-scrollbar-light shadow-inner min-h-0 scrollable-panel">
                  {teams.length === 0 ? (
                    <div className="w-full h-[calc(100vh-100px)] flex flex-col items-center justify-center opacity-40 border-2 border-dashed border-slate-200 rounded-xl py-10">
                       <span className="text-3xl mb-3">👥</span>
                       <p className="text-sm font-black uppercase tracking-widest italic text-slate-400">참가자를 추가해주세요</p>
                       <p className="text-[0.625rem] font-bold text-slate-300 mt-1 italic">* {matchMode === 'team' ? '최소 2팀 필요' : '최소 1명 필요'}</p>
                    </div>
                  ) : (
                    teams.map((t, idx) => (
                      <div key={idx} className="h-12 rounded-full border bg-white border-slate-200 text-slate-700 pl-2 pr-4 flex items-center gap-2 font-bold text-sm shadow-sm hover:border-indigo-500 transition-all animate-in zoom-in-95">
                         <div className="w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center text-white text-[0.625rem] font-black italic">#{idx + 1}</div>
               ...
               <p className="text-[0.625rem] font-bold text-slate-400 mt-4 italic ml-2">
                ...
                <div className="flex flex-col flex-1 min-h-0 gap-3">
                   ...
                   <div className="bg-slate-50 border border-slate-100 rounded-2xl p-3 shrink-0">
                      <div className="flex items-center justify-between mb-2">
                         ...
                         <span className="text-xl font-[1000] italic text-indigo-600">{progressPercent}%</span>
                      </div>
                      <div className="w-full h-2 bg-white rounded-full overflow-hidden shadow-inner">
                         <div className="h-full bg-gradient-to-r from-indigo-400 to-indigo-600 transition-all duration-700" style={{ width: `${progressPercent}%` }} />
                      </div>
                   </div>
                   ...
                   <div className="bg-slate-50/30 border border-slate-100 rounded-2xl p-3 shrink-0">
                      <h3 className="text-[0.625rem] font-black text-indigo-600 uppercase tracking-widest italic mb-2 flex items-center gap-2">
                         <span className="w-1 h-3 bg-indigo-500 rounded-full" /> MISSION GUIDE
                      </h3>
                      <div className="space-y-1.5">
                         <p className="text-[0.625rem] font-bold text-slate-500 leading-tight tracking-tighter">
                            진행자의 숫자를 플레이어가 맞추는 심리전
                         </p>
                         <p className="text-[0.625rem] font-bold text-slate-500 leading-tight tracking-tighter">
                            모드, 숫자, 참가자 설정 후 게임 시작
                         </p>
                      </div>
                   </div>
                   ...
                   <div className="mt-auto pt-3 border-t border-slate-100 space-y-2 shrink-0 sticky bottom-0 bg-white">
                      <button onClick={() => { setGameState('playing'); setHistory([]); setCurrentTeamIdx(0); setCurrentMin(1); setCurrentMax(maxNum); }} 
                        ...
                        className={`w-full h-[52px] rounded-2xl font-black text-lg transition-all relative overflow-hidden group
                          ${step3Done ? 'bg-emerald-600 text-white hover:bg-emerald-700 hover:scale-[1.02] active:scale-95 shadow-xl shadow-emerald-600/20' : 'bg-slate-100 text-slate-400 opacity-40 cursor-not-allowed'}`}>
                        <span className="relative z-10">{getButtonText()}</span>
                        {step3Done && <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000" />}
                      </button>
  ...
  return (
    <div className="max-w-6xl mx-auto w-full flex flex-col h-full animate-in fade-in py-2 font-sans text-slate-900 overflow-hidden">
      <div className="flex items-center justify-between mb-4 bg-white border border-slate-200 rounded-3xl p-6 shadow-sm min-h-[90px] shrink-0">
         <button onClick={() => setGameState('setup')} className="px-5 py-2.5 rounded-xl bg-slate-50 text-slate-400 font-black text-[0.625rem] uppercase tracking-[0.2em] border border-slate-100 hover:text-rose-500 transition-all font-sans">← 나가기</button>
        <div className="text-center">
           <p className="text-[0.625rem] font-black text-indigo-500 uppercase tracking-widest mb-1 italic">현재 순서</p>
        ...
        <div className="flex flex-col items-center border-l-4 border-slate-100 pl-8 transition-all">
           ...
           <div className="text-[0.5625rem] font-black text-slate-300 uppercase tracking-widest mt-1 italic">숫자 범위</div>
        </div>
      </div>

      <div className="flex-1 grid grid-cols-12 gap-4 overflow-hidden min-h-0">
         <div className="col-span-12 lg:col-span-4 bg-white rounded-[2.5rem] px-6 py-4 lg:p-10 shadow-xl border border-slate-100 flex flex-col items-center justify-center text-center overflow-hidden shrink-0">
         ...
         <div className="col-span-12 lg:col-span-8 flex flex-col bg-slate-900 rounded-[2.5rem] border border-white/5 p-6 lg:p-8 shadow-2xl overflow-hidden min-h-0">
            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar min-h-0">
               ...
               {history.length === 0 ? (
                  <div className="h-[calc(100vh-100px)] flex flex-col items-center justify-center opacity-20 text-white">
                  ...
                  history.map((h, i) => (
                     <div key={i} className={`flex items-center justify-between p-2 px-3 rounded-xl border transition-all ${h.result === 'CORRECT' ? 'bg-emerald-500 border-emerald-400 text-white shadow-lg' : 'bg-white/5 border-white/10 text-white animate-in slide-in-from-right duration-500'}`}>
                        <div className="flex items-center gap-3">
                           ...
                           <div>
                              <p className={`text-[0.5rem] font-black uppercase tracking-widest leading-none mb-1 opacity-50`}>
                                 {h.team}
                              </p>
                              <p className={`text-2xl font-[1000] italic leading-none ${h.result === 'CORRECT' ? 'text-white' : 'text-indigo-400/90'}`}>{h.value}</p>
                           </div>
                        </div>
                        <div className={`px-2.5 py-1 rounded-lg text-[0.5625rem] font-[1000] uppercase tracking-widest shadow-md ${h.result === 'CORRECT' ? 'bg-white text-emerald-500' : h.result === 'UP' ? 'bg-indigo-500 text-white ring-1 ring-indigo-400/30' : 'bg-rose-500 text-white ring-1 ring-rose-400/30'}`}>{h.result}</div>
                     </div>
                  ))
               )}
               </div>
