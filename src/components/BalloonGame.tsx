      ...
      <div className="max-w-screen-2xl mx-auto w-full p-4 md:p-[2cm] h-[calc(100vh-100px)] flex flex-col animate-in fade-in duration-500 font-sans text-slate-800 no-print overflow-x-hidden overflow-y-auto min-h-0 relative">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-3 bg-white border border-slate-200 rounded-2xl px-4 py-3 shadow-sm">
           <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="text-xl font-[1000] italic uppercase tracking-tighter text-slate-900 leading-none mb-1 whitespace-nowrap">풍선 터뜨리기 설정</h1>
           ...
           <div className="flex items-center gap-2 px-4 py-1.5 shrink-0 bg-pink-50 border border-pink-100 rounded-full">
          ...
          <div className="col-span-1 lg:col-span-7 flex flex-col gap-2 overflow-visible lg:overflow-y-auto min-h-0">
            ...
            <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-2 min-h-0">
               ...
               <div className={`bg-white border rounded-[2.5rem] px-6 py-4 shadow-sm flex-1 min-h-0 flex flex-col transition-all duration-300 ${teams.length >= (matchMode === 'team' ? 2 : 1) ? 'border-emerald-500 ring-4 ring-emerald-500/5' : 'border-slate-200'}`}>
                  ...
                  <div className="flex-1 max-h-[60vh] overflow-y-auto bg-slate-50/50 rounded-2xl border border-slate-100 p-4 flex flex-wrap content-start gap-2 custom-scrollbar-light shadow-inner min-h-[160px] scrollable-panel">
                    {teams.map((t, idx) => (
             ...
             <div className="bg-white border border-slate-200 rounded-[2.5rem] px-5 py-5 shadow-sm flex flex-col h-full relative overflow-hidden">
                <div className="flex items-center justify-between mb-4 shrink-0">
                   <h2 className="text-[0.6875rem] font-black text-slate-400 uppercase tracking-[0.3em] flex items-center gap-2">
                      <span className="w-2 h-2 rounded-full bg-pink-500 shadow-lg shadow-pink-500/50" /> SETTING STATUS
                   </h2>
                </div>
                ...
                <div className="flex flex-col flex-1 min-h-0 gap-3">
                   ...
                   <div className="bg-slate-50 border border-slate-100 rounded-2xl p-3 shrink-0">
                      <div className="flex items-center justify-between mb-2">
                         ...
                         <span className="text-xl font-[1000] italic text-pink-600">{progress}%</span>
                      </div>
                      <div className="w-full h-2 bg-white rounded-full overflow-hidden shadow-inner">
                         <div className="h-full bg-gradient-to-r from-pink-400 to-pink-600 transition-all duration-700" style={{ width: `${progress}%` }} />
                      </div>
                   </div>
                      ...
                      {['대전 모드 선택', '미션 상세 설정', '데이터/명단 등록'].map((label, i) => {
                        ...
                        return (
                          <div key={i} className="flex items-center justify-between px-3 py-2 bg-slate-50/50 border border-slate-100 rounded-xl">
                             <div className="flex items-center gap-2">
                                <div className={`w-1.5 h-1.5 rounded-full ${done ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-slate-300'}`} />
                                <span className={`text-[0.6875rem] font-black tracking-tight ${done ? 'text-slate-700' : 'text-slate-400'}`}>{label}</span>
                             </div>
                             <span className={`px-2 py-0.5 rounded-md text-[0.5625rem] font-black uppercase tracking-wider ${done ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}>
                                {done ? 'Ready' : 'Pending'}
                             </span>
                          </div>
                        );
                   ...
                   <div className="bg-slate-50/30 border border-slate-100 rounded-2xl p-3 shrink-0">
                      <h3 className="text-[0.625rem] font-black text-pink-600 uppercase tracking-widest italic mb-2 flex items-center gap-2">
                         <span className="w-1 h-3 bg-pink-500 rounded-full" /> MISSION GUIDE
                      </h3>
                      <div className="space-y-1.5">
                         <p className="text-[0.625rem] font-bold text-slate-500 leading-tight tracking-tighter">
                            풍선을 터뜨려 단어 빈칸을 채우는 챌린지
                         </p>
                         <p className="text-[0.625rem] font-black text-rose-500 leading-tight tracking-tighter italic">
                            <span className="mr-1">※</span> 캠 연결 필수
                         </p>
                      </div>
                   </div>
                   ...
                   <div className="mt-auto pt-3 border-t border-slate-100 space-y-2 shrink-0 sticky bottom-0 bg-white">
                      <div className="flex gap-2">
                         ...
                         <button onClick={() => fileRef.current?.click()} className="flex-1 h-[48px] text-[14px] font-semibold text-white bg-slate-900 rounded-xl hover:bg-black transition-all uppercase tracking-widest leading-none">Excel Upload 📂</button>
                      </div>
                      <input type="file" ref={fileRef} className="hidden" accept=".xlsx,.xls" onChange={handleExcel} />
                      
                      <button onClick={handleStart} 
                        disabled={!isReady}
                        className={`w-full h-[52px] rounded-2xl font-black text-lg transition-all shadow-xl relative overflow-hidden group
